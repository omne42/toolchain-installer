...
fn resolve_apt_plan_item(item: &InstallPlanItem, id: String) -> InstallerResult<ResolvedPlanItem> {
    ...
    {
        return Err(InstallerError::usage(format!(
            "plan item `{id}` with method `apt` only supports manager `apt-get`, got `{manager}`"
        )));
    }
    ...
    Ok(ResolvedPlanItem::SystemPackage(SystemPackagePlanItem {
        ...
        mode: SystemPackageMode::Explicit(
            SystemPackageManager::parse("apt-get")
                .expect("apt-get must remain a supported system package manager"),
        ),
    }))
}
