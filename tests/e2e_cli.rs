...
fn npm_global_pnpm_accepts_noop_reinstall_when_managed_metadata_matches() {
    ...
    write_executable(
        ...
        r#"#!/bin/sh
[ -n "$PNPM_HOME" ] || exit 9
pkg_root="$PNPM_HOME/pnpm-global-root/node_modules"
if [ "$1" = "root" ] && [ "$2" = "--global" ]; then
  printf '%s\n' "$pkg_root"
  exit 0
fi
pkg_dir="$pkg_root/http-server"
...
#[cfg(unix)]
#[test]
fn npm_global_pnpm_accepts_noop_reinstall_for_semver_range_spec() {
    let temp = tempfile::tempdir().expect("tempdir");
    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_pnpm = fake_bin_dir.join("pnpm");
    write_executable(
...
#[cfg(unix)]
#[test]
fn npm_global_pnpm_rejects_noop_reinstall_when_metadata_version_mismatches() {
    let temp = tempfile::tempdir().expect("tempdir");
    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_pnpm = fake_bin_dir.join("pnpm");
    write_executable(
...
if [ ! -f "$pkg_dir/package.json" ]; then
  cat > "$pkg_dir/package.json" <<'EOF'
{"name":"http-server","version":"14.1.2","bin":{"http-server":"bin/http-server"}}
EOF
  cat > "$pkg_dir/bin/http-server" <<'EOF'
#!/bin/sh
echo "14.1.2"
...
"#,
    ...
    let args = [
        ...
        "npm_global",
        "--id",
        "http-server-pnpm-version-mismatch",
        "--package",
        "http-server@14.1.1",
        "--binary-name",
        "http-server",
...
#[cfg(unix)]
#[test]
fn npm_global_npm_accepts_noop_reinstall_for_alias_spec() {
    let temp = tempfile::tempdir().expect("tempdir");
    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_npm = fake_bin_dir.join("npm");
    write_executable(
        ...
        r#"#!/bin/sh
[ -n "$npm_config_prefix" ] || exit 9
package_dir="$npm_config_prefix/lib/node_modules/alias"
mkdir -p "$package_dir/bin" "$npm_config_prefix/bin"
if [ ! -f "$package_dir/package.json" ]; then
  cat > "$package_dir/package.json" <<'EOF'
{"name":"http-server","version":"14.1.1","bin":{"http-server":"bin/http-server"}}
...
EOF
  chmod +x "$package_dir/bin/http-server"
  ln -sfn ../lib/node_modules/alias/bin/http-server "$npm_config_prefix/bin/http-server"
fi
"#,
    );
...
#[cfg(unix)]
#[test]
fn npm_global_explicit_source_accepts_noop_reinstall_when_only_receipt_remains() {
    let temp = tempfile::tempdir().expect("tempdir");
    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_npm = fake_bin_dir.join("npm");
    write_executable(
...
#[cfg(unix)]
#[test]
fn npm_global_pnpm_accepts_noop_reinstall_when_package_probe_is_unavailable() {
    let temp = tempfile::tempdir().expect("tempdir");
    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_pnpm = fake_bin_dir.join("pnpm");
    write_executable(
...
mkdir -p "$PNPM_HOME"
if [ ! -f "$PNPM_HOME/http-server" ]; then
  cat > "$PNPM_HOME/http-server" <<'EOF'
#!/bin/sh
echo "14.1.1"
EOF
...
"#,
    ...
    let args = [
        ...
        "npm_global",
        "--id",
        "http-server-pnpm-probe-failure",
        "--package",
        "http-server@14.1.1",
        "--binary-name",
        "http-server",
