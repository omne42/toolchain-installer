...
- `system_package`
  ...
  - 若调用方想固定使用 Debian/Ubuntu 的 canonical manager，应显式传 `manager=apt-get`，而不是改用不存在的 `method=apt`。
- `pip`
...
- `method=apt` 当前不属于受支持契约；需要 `apt-get` 时应使用 `method=system_package` 并显式传 `manager=apt-get`。
