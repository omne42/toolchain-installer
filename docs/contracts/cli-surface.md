...
- `0`
  - 执行成功；若未开启 `--strict`，多项执行允许部分 item 失败并仍返回 `0`。
- `2`
  - 参数错误、不支持的参数组合，不支持的 target triple，plan 中出现未知字段，或 plan / `--method` 中出现未知方法名。
- `3`
  - 单项执行中的下载或校验失败；这里的“单项执行”既包括 direct `--method`，也包括只包含一个 item 的 plan。
- `4`
  - 单项执行中的安装、解压、落盘或宿主安装失败；这里的“单项执行”既包括 direct `--method`，也包括只包含一个 item 的 plan。
- `5`
