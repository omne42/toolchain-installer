...
- 参与条件
  - 当前只有 `country=CN` 且目标 URL 的 canonical path 精确匹配 `https://github.com/git-for-windows/git/releases/download/<tag>/<asset>` 这类 `git-for-windows` release 资产时，才会生成网关候选；query/fragment 会在推断前被剥离，但 `<tag>` / `<asset>` 仍必须各自是单一路径段。
  - 若只提供 `gateway-base` 而没有国家码，不会强行启用网关候选。
  - 自定义 mirror URL、代理 URL，或仅仅在 query/path 里“包含那段字符串”的普通下载地址，都不会被静默改写成网关候选。
- 回退规则
  - 当网关候选被启用时，下载顺序会先尝试 `gateway`，再退回 direct canonical URL，最后再尝试显式 mirror；这里的 `gateway` 只是同一 canonical git release 资产的固定路由入口，不是独立来源体系。
