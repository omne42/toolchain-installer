...
- `src/install_plan/`
  ...
  - `system_package_item_execution.rs`：`plan.method=system_package` 的系统包执行，消费 runtime system-package primitives 生成 recipe；需要固定 canonical `apt-get` 时由 `manager=apt-get` 收口。
