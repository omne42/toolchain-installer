...
- `src/bootstrap/`
  - `builtin_tools/`
    - `bootstrap_flow.rs`：默认 `bootstrap` 流程与内置工具安装编排。
    - `tool_catalog.rs`：内置工具集合与默认工具列表归一化。
    - `public_release_installation.rs`：`gh` / `git` public release 资产选择与安装适配。
...
- `src/managed_toolchain/`
  ...
  - `layout.rs`：`managed_dir` 周边的 `uv` 工具目录、Python 目录、缓存目录和可执行目标路径布局。
  - `bootstrap_item_builders.rs`：托管工具链安装成功后的 `BootstrapItem` 组装与 detail 归并。
  - `source_candidate_attempts.rs`：按候选安装来源顺序尝试执行，并聚合失败信息。
  - `managed_uv_installation.rs`：确保托管 `uv` 已安装，并处理 `plan.method=uv` 的结果归并。
  - `managed_python_binary_discovery.rs`：在 `managed_dir` 中定位满足版本要求的托管 Python 可执行文件。
...
- `../omne-runtime/crates/omne-host-info-primitives/`
  - 通用宿主平台识别、目标三元组映射、home 目录解析与目标可执行后缀原语。
  - `toolchain-installer` 不再自己维护宿主 OS/arch 到 target triple 的映射。
...
- 按领域拆目录，不按技术动作堆砌杂项文件。
- 文件名必须能直接透露职责，例如 `system_package_item_execution.rs`、`release_installation.rs`、`plan_method.rs`。
