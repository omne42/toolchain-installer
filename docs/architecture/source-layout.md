...
- `src/install_plan/`
  ...
  - `system_package_item_execution.rs`：`plan.method=system_package` 的系统包执行，消费 runtime system-package primitives 生成 recipe；若调用方需要 Debian/Ubuntu 的 canonical manager，可在该方法下显式传 `manager=apt-get`。
...
- `src/managed_toolchain/`
  ...
  - `managed_uv_host_execution.rs`：托管 `uv` 子流程执行辅助，负责在调用前移除继承的宿主 `UV_*` 环境变量，再通过 shared runtime 的 process primitives 施加有界输出与硬超时执行。
