...
- `src/plan_items.rs`
  - `plan` 与 `managed_toolchain` 共享的强类型安装项模型，例如 `ResolvedPlanItem`、`UvPythonPlanItem`、`UvToolPlanItem`。
...
- `tools/source-layout-check/`
  ...
  - 同时校验顶层 crate 模块依赖方向，阻止 `managed_toolchain` 之类的下层模块反向依赖 `plan` 之类的上层编排域。
